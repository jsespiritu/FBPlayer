...
*/		
		public function hideCloseButton():void {
			if(_config != null) {
				_config.closeButton.enabled = false;
			}	
		...
		public function set html(htmlText:String):void {
			if(_config != null) {
				...
				createTextField();
				arrangeCloseButton();
				doLog("Set region HTML to " + html, Debuggable.DEBUG_REGION_FORMATION);				
			}
		}
		...
		public function loadDisplayContent(url:String, allowDomains:String, blockMouseActions:Boolean=false):void {
		  	 clearDisplayContent();
			 ...
			 var urlReq:URLRequest = new URLRequest(url);
		 	 _contentLoader.contentLoaderInfo.addEventListener(Event.INIT,
			 	function(event:Event):void {
					doLog("External SWF successfully loaded from " + url + " (" + _contentLoader.width + "x" + _contentLoader.height + ")", Debuggable.DEBUG_REGION_FORMATION);
				}
			 );
			 _contentLoader.mouseChildren = !blockMouseActions; /* was false */
			 _contentLoader.mouseEnabled = !blockMouseActions; /* was false */
		...
		public function clearDisplayContent():void {
			if(_contentLoader != null) {
				try {		
					...
					doLog("** loader.unload() called on display content (e.g. SWF ad)", Debuggable.DEBUG_DISPLAY_EVENTS);
				}
		...
		private function arrangeText():void {
			if(_text) {
				...
				doLog("Arranging text (" + _text.text + ") to sit at X:" + _text["y"] + " Y:" + _text["x"] + " height:" + _text["height"] + " width:" + _text["width"], Debuggable.DEBUG_REGION_FORMATION);
			}
		}
		...
		protected function onClick(event:MouseEvent):void {
			...
			if(!_config.keepVisibleAfterClick && autoHide) hide();
