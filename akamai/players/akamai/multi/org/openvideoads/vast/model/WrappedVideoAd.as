...
package org.openvideoads.vast.model {
	...
	public class WrappedVideoAd extends VideoAd implements TemplateLoadListener {
		...
		public function WrappedVideoAd(originalAdId:Object, wrapperXML:XML=null, vastContainer:VideoAdServingTemplate=null) {
			super();
		...
		public function onTemplateLoaded(template:VideoAdServingTemplate):void {
			doLog("XML Wrapper: VAST data loaded for original ad " + _originalAdId.id + " loaded - signalling load completion", Debuggable.DEBUG_VAST_TEMPLATE);
		...
		public function onTemplateLoadError(event:Event):void {
			doLog("XML Wrapper: Failure obtaining VAST data for original ad " + _originalAdId.id), Debuggable.DEBUG_VAST_TEMPLATE;
		...
		public override function isInteractive(deliveryType:String, mimeType:String='any', bitrate:* = -1, width:int = -1, height:int = -1):Boolean {
			if(hasReplacementVideoAd()) {
				return _videoAdReturnedFromWrapper.isInteractive(deliveryType, mimeType, bitrate, width, height);											
			}
			return super.isInteractive(deliveryType, mimeType, bitrate);
		}
		...
		public override function getStreamToPlay(deliveryType:String, mimeType:String, bitrate:* = -1, width:int = -1, height:int = -1):NetworkResource {
			if(hasReplacementVideoAd()) {
				return _videoAdReturnedFromWrapper.getStreamToPlay(deliveryType, mimeType, bitrate, width, height);
			}
			else return super.getStreamToPlay(deliveryType, mimeType, bitrate, width, height);
		}
		...
		public override function triggerForcedImpressionConfirmations(overrideIfAlreadyFired:Boolean=false):void {
			if(hasReplacementVideoAd()) {
				_videoAdReturnedFromWrapper.triggerForcedImpressionConfirmations(overrideIfAlreadyFired);
			}
