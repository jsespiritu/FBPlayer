...
package org.openvideoads.vast.schedule.ads {
	...
	public class AdSchedule extends Debuggable implements TemplateLoadListener {
							...
							if(adSpot.zone && adSpot.zone.toUpperCase() == "STATIC") {
								...
								originalAdSlot = new AdSlot(relatedStreamSequence,
													 ...
													 ((adSpot.companionDivIDs == undefined) ? 
										  	 		 ...
										  	 		 ((adSpot.bestBitrate != undefined) ? adSpot.bestBitrate : config.bestBitrate),
		...
		public function closeActiveOverlaysAndCompanions():void {
			...
			if(hasAdSlots()) {
				for(var i:int = 0; i < _adSlots.length; i++) {
					if(_adSlots[i].isPlaying()) _adSlots[i].closeActiveOverlaysAndCompanions();
				}
			}
		}        

		public function closeOutdatedOverlaysAndCompanionsForThisStream(streamIndex:int, milliseconds:Number):void {
			// used to clear up any active overlays or companions if the current stream is skipped
			if(hasAdSlots()) {
				for(var i:int = 0; i < _adSlots.length; i++) {
					if(_adSlots[i].associatedStreamIndex == streamIndex && !_adSlots[i].isLinear()) {
						if(_adSlots[i].isPlaying() && !_adSlots[i].shouldBePlaying(milliseconds)) {
							_adSlots[i].closeActiveOverlaysAndCompanions();
						} 					
					}
				}
			}
